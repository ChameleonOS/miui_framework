...
public class RemoteViewsAdapter extends BaseAdapter
    ...
    private class FixedSizeRemoteViewsCache {
        ...
        private int getFarthestPositionFrom(int i) {
            int j = 0;
            int k = -1;
            int l = 0;
            int i1 = -1;
            ...
            do {
                ...
                if(k1 > l && !((RemoteViewsIndexMetaData)mIndexMetaData.get(Integer.valueOf(j1))).isRequested) {
                    i1 = j1;
                    l = k1;
                }
                if(k1 >= j) {
...
_L3:
        ...
        public void insert(int i, RemoteViews remoteviews, long l, boolean flag) {
            if(mIndexRemoteViews.size() >= mMaxCount)
                mIndexRemoteViews.remove(Integer.valueOf(getFarthestPositionFrom(i)));
            int j;
            if(mLastRequestedIndex > -1)
                j = mLastRequestedIndex;
            else
                j = i;
            for(; getRemoteViewsBitmapMemoryUsage() >= 0x200000; mIndexRemoteViews.remove(Integer.valueOf(getFarthestPositionFrom(j))));
            if(mIndexMetaData.containsKey(Integer.valueOf(i)))
                ((RemoteViewsIndexMetaData)mIndexMetaData.get(Integer.valueOf(i))).set(remoteviews, l, flag);
            else
                mIndexMetaData.put(Integer.valueOf(i), new RemoteViewsIndexMetaData(remoteviews, l, flag));
...
_L4:
    ...
    private class RemoteViewsIndexMetaData {
        ...
        public void set(RemoteViews remoteviews, long l, boolean flag) {
            itemId = l;
            if(remoteviews != null)
        ...
        public RemoteViewsIndexMetaData(RemoteViews remoteviews, long l, boolean flag) {
            this$0 = RemoteViewsAdapter.this;
            super();
            set(remoteviews, l, flag);
        }
    }

    private class RemoteViewsMetaData {

        private RemoteViewsFrameLayout createLoadingView(int i, View view, ViewGroup viewgroup) {
...
_L1:
            ...
            mFirstView = null;
_L4:
            TextView textview = (TextView)mLayoutInflater.inflate(0x1090092, remoteviewsframelayout, false);
...
_L2:
            ...
            Exception exception1;
            exception1;
            mFirstViewHeight = Math.round(50F * mContext.getResources().getDisplayMetrics().density);
            mFirstView = null;
    ...
    private class RemoteViewsFrameLayoutRefSet {
        ...
        public void notifyOnRemoteViewsLoaded(int i, RemoteViews remoteviews, int j) {
...
*/
    ...
    private void loadNextIndexInBackground() {
        mWorkerQueue.post(new Runnable() {
            ...
            public void run() {
                if(!mServiceConnection.isConnected()) goto _L2; else goto _L1
_L1:
                FixedSizeRemoteViewsCache fixedsizeremoteviewscache = mCache;
                fixedsizeremoteviewscache;
                JVM INSTR monitorenter ;
                int i;
                boolean flag;
                int ai[] = mCache.getNextIndexToLoad();
                i = ai[0];
                if(ai[1] <= 0)
...
_L2:
    ...
    private void onNotifyDataSetChanged() {
        ...
        RuntimeException runtimeexception;
        RemoteException remoteexception;
        for(int j = mVisibleWindowLowerBound; j <= mVisibleWindowUpperBound; j++)
            if(j < i)
                updateRemoteViews(j, false, false);
...
_L2:
    ...
    private void updateRemoteViews(final int position, boolean flag, boolean flag1) {
...
_L2:
        ...
        fixedsizeremoteviewscache;
        JVM INSTR monitorenter ;
        mCache.insert(position, rv, l, flag);
...
_L16:
        ...
        RemoteViewsIndexMetaData remoteviewsindexmetadata = mCache.getMetaDataAt(i);
        remoteviewsindexmetadata.isRequested = true;
        k = remoteviewsindexmetadata.typeId;
...
_L12:
        ...
        view2.setTagInternal(0x102023a, integer);
...
_L15:
        ...
        synchronized(mCache.getMetaData()) {
            remoteviewsframelayout = remoteviewsmetadata1.createLoadingView(i, view, viewgroup);
        }
        if(flag2)
            loadNextIndexInBackground();
        fixedsizeremoteviewscache;
...
_L4:
        synchronized(mCache.getMetaData()) {
            remoteviewsframelayout = remoteviewsmetadata.createLoadingView(i, view, viewgroup);
        }
